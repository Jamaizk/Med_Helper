...
local fmbuttons = {
	...
	{name = u8'Мед.карта', rank = 3},
	{name = u8'Страховка', rank = 3},
	{name = u8'Военник', rank = 4},
			...
			imgui.Begin(u8'Меню быстрого доступа', windows.imgui_fm, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar)
				...
				imgui.BeginChild('##fmmainwindow', imgui.ImVec2(300, -1), false)
					if newwindowtype[0] == 1 then
						if clienttype[0] == 0 then
							...
							imgui.BeginGroup()
								...
								if configuration.main_settings.myrankint >= 1  then
									if imgui.Button(fa.ICON_FA_FILE_ALT..u8' Вылечить игрока без рп', imgui.ImVec2(285,30)) then
										sendchatarray(configuration.main_settings.playcd, {
											{'/heal %s %s',fastmenuID, configuration.main_settings.heal},
										})
									end
								...
								if configuration.main_settings.myrankint >= 2  then
									...
									imgui.Hint('firstrangpricelist', 'С 2-го ранга')
								end
								...
								if configuration.main_settings.myrankint >= 3 then
									...
									imgui.Hint('secondrangexpel', 'Со 3-го ранга')
								end
							imgui.EndGroup()				
						end
					...
					elseif newwindowtype[0] == 2 then
						...
						imgui.SetCursorPos(imgui.ImVec2(15,20))
						if voenka[0] == 0 then
							imgui.TextColoredRGB('Военник: Этап 1',1)
							imgui.Separator()
							imgui.SetCursorPosX(7.5)
							if imgui.Button(fa.ICON_FA_HAND_PAPER..u8' Узнать нужен ли откос', imgui.ImVec2(285,30)) then
								getmyrank = true
								sampSendChat('/stats')
								if tonumber(os.date('%H', os.time(os.date('!*t')) + 2 * 60 * 60)) > 4 and tonumber(os.date('%H', os.time(os.date('!*t')) + 2 * 60 * 60)) < 13 then
									sendchatarray(configuration.main_settings.playcd, {
										{'/c Здравствуйте, вам нужно помочь не идти в армию?'},
									})
								elseif tonumber(os.date('%H', os.time(os.date('!*t')) + 2 * 60 * 60)) > 12 and tonumber(os.date('%H', os.time(os.date('!*t')) + 2 * 60 * 60)) < 17 then
									sendchatarray(configuration.main_settings.playcd, {
										{'/c Здравствуйте, вам нужно помочь не идти в армию?'},
							...
							imgui.SetCursorPosX(7.5)
							imgui.Button(u8'Просить проследовать за мной ', imgui.ImVec2(285,30))
							if imgui.IsItemHovered() then
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										sendchatarray(configuration.main_settings.playcd, {
							...
							imgui.SetCursorPosX(7.5)
							imgui.Button(u8'Отыгровка ', imgui.ImVec2(285,30))
							if imgui.IsItemHovered() then
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										sendchatarray(configuration.main_settings.playcd, {
											{'Сейчас я проведу проведу вам мед.осмотр,который покажет можно вам идти в армию или нет.'},
											{'Раздевайтесь.'},
											{'/me достал стетоскоп'},
											{'/do Стетоскоп в руках.'},
											{'/me приложил стетоскоп к груди пациента'},
											{'/todo У вас учащенное сердцебиение*убирая стетоскоп'},
											{'Давайте померяю вам давление'},
											{'/me достал тонометр'},
											{'Засучите рукав.'},
											{'/me надел манжету на плечо пациента'},
											{'/me начал затягивать манжету'},
											{'/do Манжета затянута.'},
											{'/me запустил тонометр'},
											{'/do Тонометр накачивает воздух.'},
											{'/do Воздух накачен.'},
											{'/do Тонометр медленно начал спускать воздух.'},
											{'/do На экране тонометра написано 156/92 пульс 121.'},
											{'У вас ужасное давление, с таким вам в армию категорически запрещено.'},
											{'/me взял бумаги'},
											{'/me написал бумагу об отстранении человека от службы в армии'},
											{'/me поставил печать больницы'},
											{'/do Печать стоит.'},
											{'/todo Удачи вам и доброго пути*передавая бумагу человеку напротив'},
											{'/mticket %s ', fastmenuID},
										})
									else
										MedHelperMessage('Не торопитесь, Вы уже отыгрываете что-то! Прервать отыгровку: {MC}page down{WC}')
									end
						...
						if voenka[0] ~= 0 then
							if imgui.InvisibleButton('##medbackbutton',imgui.ImVec2(55,15)) then
								if voenka[0] ~= 0 then voenka[0] = voenka[0] - 1
								end
							end
							imgui.SetCursorPos(imgui.ImVec2(15,240))
							imgui.PushFont(font[16])
						...
						imgui.SetCursorPosY(240)
						if voenka[0] ~= 5 and voenka[0] ~= 2 then
							imgui.SetCursorPosX(195)
							if imgui.InvisibleButton('##medforwardbutton',imgui.ImVec2(125,15)) then
								voenka[0] = voenka[0] + 1
							end
